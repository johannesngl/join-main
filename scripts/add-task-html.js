 ...
 */
function renderSearchNameFromList(element, initial) {
  return /* html */ `
    <div class="searchNameWrapper">
      <input type="checkbox" id="checkbox_${element.name}" name="optionen" value="${element.name}">
      <label for="checkbox_${element.name}">
        <div class="checkboxName">
          <div class="userInitial" style="background-color: ${element.color}">${initial}</div>
          <p>${element.name}</p>
        </div>
      </label>
    </div>
...
function renderGetSubtasks(i, element) {
  return /* html */ `
    <div class="showTaskSubtaskEditBTN" id="showTaskSubtaskEditBTN${i}">
        <label for="showTaskSubtaskEditInput${i}" class="editSubtaskLabel">Edit Subtask:</label>
