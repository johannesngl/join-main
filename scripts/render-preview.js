...
function renderPreviewCardHeader(index) {
  let previewCardHTML = `<div class="preview-card-header preview-card-menu no-side-margin">
  ...
  if (task[index].assignetTo) {
    for (let i = 0; i < task[index].assignetTo.length; i++) {
      if (contact[task[index].assignetTo[i]]) {
        let participiantInitials = getInitials(
          contact[task[index].assignetTo[i]].name
        );
        previewCardHTML += `<div class="user-preview">                                
                                <div style="background-color: ${
                                  contact[task[index].assignetTo[i]].color
                                }" class="user-in left-20">${participiantInitials}</div>
                                <div>${
                                  contact[task[index].assignetTo[i]].name
                                }</div>
                            </div>`;
...
function renderEditCardPriority(index) {
  ...
  let editCardForm = `<div class="title-input">                        
                            ...
                            <div id="priority">
                                <div id="btn-prio-low" onclick="changePriority('low')" class="btn-prio ${
                                  priority === "low" ? "green" : ""
                                }" data-value="low">
                                    <span>Low</span>
                                ...
                                <div id="btn-prio-medium" onclick="changePriority('medium')"  class="btn-prio ${
                                ...
                                <div id="btn-prio-high"  onclick="changePriority('high')" class="btn-prio ${
                                  priority === "high" ? "red" : ""
                                }" data-value="high">
                                    <span>High</span>
...
function renderEditCardParticipiants(index) {
  ...
  for (let i = 0; i < contactId.length; i++) {
    ...
    if (contactIsAssignet == true) {
      editCardForm += `<label><input type="checkbox" name="task-participiant" value="${
        contactId[i]
      }" checked>${contact[contactId[i]].name}</label>`;
    } else {
      editCardForm += `<label><input type="checkbox" name="task-participiant" value="${
        contactId[i]
      }">${contact[contactId[i]].name}</label>`;
    }
    editCardForm += `<br>`;
  }
  editCardForm += `   </div>
      ...
      editCardForm += `<div id="subtask-${i}">
                                <div class="subtask-preview" id="subtask-preview-${i}">
                                    <div class="subtask-description">
                                        <input onclick="updateSubtaskState(${i})" class="task-done-prev" type="checkbox" id="checkbox-${i}" name="subtask-${i}" ${
        isChecked ? "checked" : ""
      }>
                                        <span id="description-preview-${i}" class="description-preview">${
        task[index].subtask[i].description
      }</span>
                                    </div>
                                ...
                                <div class="subtask-edit d-none" id="subtask-edit-${i}">
                                    <div class="subtask-description">                                        
                                        <input class="description-preview" type="text" id="subtask-description-${i}" value="${
        task[index].subtask[i].description
      }">                                    
                                    </div>
                                    <div class="edit-icons">
