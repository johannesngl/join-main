...
function filterTasks() {
  ...
  for (let i = 0; i < taskId.length; i++) {
    if (
      !task[taskId[i]].title.toLowerCase().includes(searchString) &&
      !task[taskId[i]].description.toLowerCase().includes(searchString)
    ) {
...
function addDragAndDropListeners(element) {
  if (
    !element.touchStartAdded ||
    !element.touchmoveAdded ||
    !element.touchendAdded ||
    !element.touchendAdded
  ) {
    let offsetX, offsetY;
    let initialParent;
...
function dishighlight(state) {
  ...
  if (document.getElementById(`no-task-to-do-${state}`)) {
    document
      .getElementById(`no-task-to-do-${state}`)
