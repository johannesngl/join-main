...
async function logIn() {
  ...
  if (!userDataCorrect) {
    document.getElementById(
      "wrongPassword"
    ).innerHTML = `Email address or password is incorrect`;
  }
}
...
async function createNewUser() {
  ...
  document
    .getElementById("passwordsDontMatchNotification")
    .classList.add("dont-show");
  document
    .getElementById("emailAlreadyExistsNotification")
    .classList.add("dont-show");

  if (password !== confirmPassword) {
    document.getElementById("password").style.border =
      "solid 2px rgb(252, 3, 3)";
...
function confirmRegistration() {
  document.getElementById("logInBtns").innerHTML =
    '<button class="btn-dark btn confirm-registration" style="font-size: 21px;" disabled><p>You Signed Up successfully</p></button>';
  setTimeout(function () {
